    ...
    <main>
            ...
            <div class="container margin_120_95">
                ...
                <div id="reccomended" class="owl-carousel owl-theme">
                    @foreach($doctors_list as $doctor)
                        <div class="item">
                            <a href="{{ route('front.doctors.view', ['spec' => $doctor]) }}">
                                <div class="views"><i class="icon-eye-7"></i></div>
                                <div class="title">
                                    <h4>{{ __('vars.dr') }}<em>{{ $doctor->full_name }} - {{ $doctor->category->title }}</em></h4>
        ...
        <div class="container margin_120_95">
            ...
            <div class="row justify-content-center">
                @if($clinics_list->count() > 0)
                    <div class="col-xl-4 col-lg-5 col-md-6">
                        <div class="list_home">
                            <div class="list_title">
                                <i class="icon_pin_alt"></i>
                                <h3>{{ __('vars.top_clinics') }}</h3>
                            </div>
                            <ul>
                                @foreach($clinics_list as $clinic)
                                    <li><a href="{{ route('front.clinics.view', ['clinic' => $clinic]) }}">
                                            {{ $clinic->title }}
                                        </a>
                                    </li>
                                @endforeach
                                <li><a href="{{ route('front.clinics') }}">{{ __('vars.all_clinics') }}</a></li>
                            </ul>
                        </div>
                    </div>
                @endif
                    ...
                    <div class="col-xl-4 col-lg-5 col-md-6">
                        <div class="list_home">
                            <div class="list_title">
                                ...
                                <h3>{{ __('vars.categories') }}</h3>
                            </div>
                            <ul>
                                @foreach($categories_list as $category)
                                    <li><a href="{{ route('front.categories.view', ['category' => $category]) }}">
                                            {{ __($category->title) }}
                                        </a>
                                    </li>
                                @endforeach
                                <li><a href="{{ route('front.services') }}">{{ __('vars.all_categories') }}</a></li>
                            </ul>
                        </div>
                    </div>
                @endif
